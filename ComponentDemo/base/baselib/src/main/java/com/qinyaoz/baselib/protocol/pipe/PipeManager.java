...
import com.qinyaoz.baselib.protocol.pipe.base.IPipe;
...
public final class PipeManager {
    ...
    private static Map<Class, IPipe> pipes = new HashMap<>();
    public static  <T extends IPipe> T get (Class<T> clz) {
        if (!clz.isInterface()) {
     ...
     * 注册Pipe实例到对应的协议接口上。
     */
    public static <T extends IPipe> void register (Class<T> clz,T t) {
    ...
    private static <T extends IPipe> T findPipeFromMap(Class<T> clz) {
    ...
    private static <T extends IPipe> void putIntoPipeMap (Class<T> clz,T t) {
        pipes.remove(clz);
        //noinspection unchecked
        pipes.put(clz, t);
    }
    ...
    private static class DefaultInvocationHandler implements InvocationHandler {
        ...
        private void triggerCallback(Object[] args) {
            for (int i = 0; i < (args == null ? 0 : args.length); i++) {
                Object arg = args[i];
                if (arg instanceof PipeCallback) {
