...
public class TabLayout extends HorizontalScrollView {
    ...
    public @interface TabIndicatorMode {
    }
    ...
    private OnCreateTabCustomViewListener onCreateTabCustomViewListener;
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        super.addView(slidingTabIndicator, 0, new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));

        TypedArray a = ThemeEnforcement.obtainStyledAttributes(context, attrs, R.styleable.cui_TabLayout, defStyleAttr, R.style.Widget_Design_TabLayout);

        slidingTabIndicator.setSelectedIndicatorHeight(a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabIndicatorHeight, -1));

        slidingTabIndicator.setSelectedIndicatorWidth(a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabIndicatorWidth, -1));

        slidingTabIndicator.setSelectedIndicatorColor(a.getColor(R.styleable.cui_TabLayout_cui_tabIndicatorColor, 0));
        setSelectedTabIndicator(MaterialResources.getDrawable(context, a, R.styleable.cui_TabLayout_cui_tabIndicator));
        setSelectedTabIndicatorGravity(a.getInt(R.styleable.cui_TabLayout_cui_tabIndicatorGravity, INDICATOR_GRAVITY_BOTTOM));
        setTabIndicatorFullWidth(a.getBoolean(R.styleable.cui_TabLayout_cui_tabIndicatorFullWidth, true));
        setUseTabIndicatorColor(a.getBoolean(R.styleable.cui_TabLayout_cui_tabUseIndicatorColor, true));


        setSelectedTabIndicatorMode(a.getInt(R.styleable.cui_TabLayout_cui_tabIndicatorMode, INDICATOR_LINE));

        tabPaddingStart = tabPaddingTop = tabPaddingEnd = tabPaddingBottom = a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabPadding, 0);
        tabPaddingStart = a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabPaddingStart, tabPaddingStart);
        tabPaddingTop = a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabPaddingTop, tabPaddingTop);
        tabPaddingEnd = a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabPaddingEnd, tabPaddingEnd);
        tabPaddingBottom = a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabPaddingBottom, tabPaddingBottom);

        tabTextAppearance = a.getResourceId(R.styleable.cui_TabLayout_cui_tabTextAppearance, R.style.TextAppearance_Design_Tab);

        // Text colors/sizes come from the text appearance first
        final TypedArray ta = context.obtainStyledAttributes(tabTextAppearance, android.support.v7.appcompat.R.styleable.TextAppearance);
        try {
            tabTextSize = ta.getDimensionPixelSize(android.support.v7.appcompat.R.styleable.TextAppearance_android_textSize, 0);
            tabTextColors = MaterialResources.getColorStateList(context, ta, android.support.v7.appcompat.R.styleable.TextAppearance_android_textColor);
        } finally {
            ta.recycle();
        }

        if (a.hasValue(R.styleable.cui_TabLayout_cui_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            tabTextColors = MaterialResources.getColorStateList(context, a, R.styleable.cui_TabLayout_cui_tabTextColor);
        }

        if (a.hasValue(R.styleable.cui_TabLayout_cui_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.cui_TabLayout_cui_tabSelectedTextColor, 0);
            tabTextColors = createColorStateList(tabTextColors.getDefaultColor(), selected);
        }

        tabIconTint = MaterialResources.getColorStateList(context, a, R.styleable.cui_TabLayout_cui_tabIconTint);
        tabIconTintMode = ViewUtils.parseTintMode(a.getInt(R.styleable.cui_TabLayout_cui_tabIconTintMode, -1), null);

        tabRippleColorStateList = MaterialResources.getColorStateList(context, a, R.styleable.cui_TabLayout_cui_tabRippleColor);

        requestedTabMinWidth = a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabMinWidth, INVALID_WIDTH);
        requestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabMaxWidth, INVALID_WIDTH);
        tabBackgroundResId = a.getResourceId(R.styleable.cui_TabLayout_cui_tabBackground, 0);
        contentInsetStart = a.getDimensionPixelSize(R.styleable.cui_TabLayout_cui_tabContentStart, 0);
        // noinspection WrongConstant
        mode = a.getInt(R.styleable.cui_TabLayout_cui_tabMode, MODE_FIXED);
        tabGravity = a.getInt(R.styleable.cui_TabLayout_cui_tabGravity, GRAVITY_FILL);
        inlineLabel = a.getBoolean(R.styleable.cui_TabLayout_cui_tabInlineLabel, false);
        unboundedRipple = a.getBoolean(R.styleable.cui_TabLayout_cui_tabUnboundedRipple, false);
        a.recycle();
    ...
    public void setSelectedTabIndicatorMode(@TabIndicatorMode int indicatorMode) {
        if (tabIndicatorMode != indicatorMode) {
            tabIndicatorMode = indicatorMode;
            ViewCompat.postInvalidateOnAnimation(slidingTabIndicator);
        }
    }
    ...
    public void setIndicatorWidth(int indicatorWidth) {
        slidingTabIndicator.setSelectedIndicatorWidth(indicatorWidth);
    }

    public void setIndicatorStartInterpolator(Interpolator interpolator) {
    ...
    public void setIndicatorEndInterpolator(Interpolator interpolator) {
        if (interpolator == null) {
            interpolator = new LinearInterpolator();
        }
    ...
    void populateFromPagerAdapter() {
        ...
        if (pagerAdapter != null) {
            ...
            for (int i = 0; i < adapterCount; i++) {
                if (onCreateTabCustomViewListener != null) {
                    Tab tab = newTab();
                    View view = onCreateTabCustomViewListener.OnCreateTabView(tab.view, i);
                    if (view != null) {
    ...
    public class TabView extends LinearLayout {
        ...
        private int getContentWidth() {
            ...
            for (View view : new View[]{textView, iconView, customTextView, customIconView, customView}) {
         ...
         *
         * @return Height of the tab label, if present, or the height of the tab icon, if present. If tabs
         * is in inline mode, returns the sum of both the icon and tab label heights.
         */
    ...
    private class SlidingTabIndicator extends LinearLayout {
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0 && selectedPosition < childCount) {
                ...
                int selectedTitleWidth = right - left;


                // Draw the selection partway between the tabs
                View nextTitle = getChildAt(selectedPosition + 1 < childCount ? selectedPosition + 1 : selectedPosition);
                ...
                int nextTitleWidth = nextTitleRight - nextTitleLeft;


                if (selectedIndicatorWidth > 0 && !tabIndicatorFullWidth) {
                    selectedIndicatorWidth = Math.min(selectedTitleWidth, selectedIndicatorWidth);
                    selectedIndicatorWidth = Math.min(nextTitleWidth, selectedIndicatorWidth);

                    left = left + (selectedTitleWidth - selectedIndicatorWidth) / 2;
                    nextTitleLeft = nextTitleLeft + (nextTitleWidth - selectedIndicatorWidth) / 2;
                    right = left + selectedIndicatorWidth;
                    nextTitleRight = nextTitleLeft + selectedIndicatorWidth;

                }

                left = (int) (left + (nextTitleLeft - left) * mStartInterpolator.getInterpolation(selectionOffset));
                right = (int) (right + (nextTitleRight - right) * mEndInterpolator.getInterpolation(selectionOffset));
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            int targetViewWidth = targetRight - targetLeft;

            if (selectedIndicatorWidth > 0 && !tabIndicatorFullWidth) {
                targetLeft = targetLeft + (targetViewWidth - selectedIndicatorWidth) / 2;
                targetRight = targetLeft + selectedIndicatorWidth;
            }
...
//                tabViewContentWidth = dpToPx(MIN_INDICATOR_WIDTH);
            ...
            int contentBottomBounds = tabViewCenterY + (tabViewContentHeight / 2);
        ...
        public void draw(Canvas canvas) {
            ...
            if (tabIndicatorMode == INDICATOR_LINE) {
            ...
            /*int temp = (indicatorRight - indicatorLeft) / 2 - 20;
            indicatorLeft += temp;
            indicatorRight -= temp;*/
    ...
    public interface OnCreateTabCustomViewListener {
        View OnCreateTabView(TabView view, int position);
    }


    public void setOnCreateTabCustomViewListener(OnCreateTabCustomViewListener onCreateTabCustomViewListener) {
        this.onCreateTabCustomViewListener = onCreateTabCustomViewListener;
    }
